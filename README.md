...
A native GUI for finding, reading, and resuming past Claude Code sessions stored under `~/.claude/projects/`, on Linux and macOS. It offers a session list grouped by project, a typed search that streams matches across all projects as snippets under each session, a docked viewer that reads a session as its foldable turns, and right-click actions that resume a session in its original working directory.
...
The **reading view** on the right is filled by the first click. A single click on a session header or a snippet renders the whole jsonl, then anchors it to the relevant line, so the view never grows or jumps as it loads. It splits a long jsonl at compaction-boundary records and at idle gaps over ten minutes, presenting each segment as a section. Within a section the transcript reads in **turns** - one typed prompt and the work that answered it. Each turn folds to its first line, and folding them all (the ⋯ menu, or the band header) leaves a table of contents; a Turns tab above the transcript jumps anywhere. Its count follows what a double-`Esc` rollback in Claude Code would list, so it can read lower than the list's Turns column, which also counts queued and harness-injected prompts. Tool calls, tool results and thinking hide behind a faint per-turn count, yet search still reaches inside them, and jumping to a hit reveals exactly the block it sits in. A hover ⧉ copies any message whole; each quoted block carries its own. `Ctrl-F` opens an inline find within the session.

Right-clicking a session offers "Open in viewer", "Copy resume command", "Copy session id", "Copy session path", "Copy last assistant output", "Copy session as Markdown", "Export to .md...", "Resume in new terminal tab", "Resume forked", "Move to...", "Reveal folder", and "Add/Remove bookmark"; right-clicking a search hit's snippet adds "Open at this match" and "Copy this snippet". A session can also be moved by dragging it onto another folder heading. The terminal launcher detects `gnome-terminal` / `konsole` / `ptyxis` / `xterm` and uses the right `--tab`-style invocation.
...
- **coachman** drives one harnessed `claude -p` CLI session for unattended, validated agent runs: stream-json invocation, usage-limit-window recovery, session resume for fix loops, stall and cost-cap watchdogs, and a per-stage cost ledger. It needs `deadman` (from the teatotal shelf) on the module path; `anthropic-rates.tcl` and `tallyman` alongside give its cost cap a live meter.
