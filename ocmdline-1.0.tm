...
oo::class create ocmdline {
    ...
    # prints; there is no third place to say it. -help is required, because an
    # option nobody can read about is the defect this module exists to make
    # unwritable.
    #
    ...
    method option {name args} {
        ...
        set spec [dict create help $help section "" arg "" suffix "" repeat 0 \
            selects "" modes {} because "" check "" guard ""]
    ...
    method reject {token message} { dict set Rejects $token $message }
    ...
    method Seal {} {
        ...
        dict for {name spec} $Options { lappend prose {*}[dict get $spec help] }
