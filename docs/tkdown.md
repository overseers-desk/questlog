...
| `prose` | `w idx text baseTags {suffix "\n\n"}` | Insert prose plus GFM pipe tables and ATX headings, closed by `suffix`. Tables render as tab-aligned columns, each under its own `td-tbl<N>` tag. |
