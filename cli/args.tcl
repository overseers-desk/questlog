...
proc ::questlog::cli::args::usage {{channel stderr} {code 2}} {
    ...
    puts $channel "  --tool:read <path-tail>       A file read."
    puts $channel "  --tool:write|edit|file <path-tail>"
    puts $channel "                                A file written / edited / touched."
    ...
    puts $channel ""
    puts $channel "A path-tail is matched from the right, as a literal, not a glob: 'scan.tcl' finds"
    puts $channel "the file in any directory, and 'lib/scan.tcl' any path ending that way. The tail"
    puts $channel "starts at a '/' or a '.', so '.tcl' and 'tcl' both find every Tcl file, while"
    puts $channel "'an.tcl' finds none."
    puts $channel ""
