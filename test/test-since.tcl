...
check parse_24h   {rel 86400}   [::questlog::scan::parse_since 24h]
check parse_7d    {rel 604800}  [::questlog::scan::parse_since 7d]
check parse_30d   {rel 2592000} [::questlog::scan::parse_since 30d]
check parse_2w    {rel 1209600} [::questlog::scan::parse_since 2w]
check parse_90m   {rel 5400}    [::questlog::scan::parse_since 90m]
check parse_empty {none}        [::questlog::scan::parse_since ""]
check parse_all   {none}        [::questlog::scan::parse_since all]
check parse_bad_unit  1 [catch {::questlog::scan::parse_since 3x}]
check parse_bad_word  1 [catch {::questlog::scan::parse_since abc}]
check parse_bad_order 1 [catch {::questlog::scan::parse_since d7}]

# ---- parse_since: absolute ISO date -> {abs <local-midnight-epoch>} --
set abs_epoch [clock scan 2026-04-01 -format "%Y-%m-%d"]
check parse_abs_kind  abs        [lindex [::questlog::scan::parse_since 2026-04-01] 0]
check parse_abs_epoch $abs_epoch [lindex [::questlog::scan::parse_since 2026-04-01] 1]
check parse_bad_day   1 [catch {::questlog::scan::parse_since 2026-02-30}]
check parse_bad_month 1 [catch {::questlog::scan::parse_since 2026-13-01}]
check parse_bad_short 1 [catch {::questlog::scan::parse_since 2026-4-1}]
...
check parse_dt_kind   absdt   [lindex [::questlog::scan::parse_since 2026-04-01T13:37] 0]
check parse_dt_hm     $dt_hm  [lindex [::questlog::scan::parse_since 2026-04-01T13:37] 1]
check parse_dt_hms    $dt_hms [lindex [::questlog::scan::parse_since 2026-04-01T13:37:30] 1]
# A space separator works when the token is quoted (same instant as the T form).
check parse_dt_space  $dt_hm  [lindex [::questlog::scan::parse_since {2026-04-01 13:37}] 1]
check parse_bad_hour  1 [catch {::questlog::scan::parse_since 2026-04-01T25:00}]
check parse_bad_min   1 [catch {::questlog::scan::parse_since 2026-04-01T13:60}]
# A bare date stays day-granular (abs), not absdt - the common case is unchanged.
check parse_dt_bare_date abs [lindex [::questlog::scan::parse_since 2026-04-01] 0]

# ---- cutoff_for: "all" => no bound; relative => now - secs; abs => epoch-1
check cutoff_all 0 [::questlog::scan::cutoff_for [dict create since all]]
set now [clock seconds]
set c7 [::questlog::scan::cutoff_for [dict create since 7d]]
check cutoff_7d_within 1 [expr {abs(($now - 604800) - $c7) <= 2}]
check cutoff_abs [expr {$abs_epoch - 1}] \
    [::questlog::scan::cutoff_for [dict create since 2026-04-01]]

# ---- ceiling_for: no until => no bound; rel => now - secs; abs => end of day
check ceiling_none "" [::questlog::scan::ceiling_for [dict create]]
check ceiling_all  "" [::questlog::scan::ceiling_for [dict create until all]]
set u7 [::questlog::scan::ceiling_for [dict create until 7d]]
...
check ceiling_abs [expr {[clock add $abs_epoch 1 day] - 1}] \
    [::questlog::scan::ceiling_for [dict create until 2026-04-01]]
# A datetime cutoff/ceiling pins to the named second (cutoff epoch-1, ceiling exact).
check cutoff_dt  [expr {$dt_hm - 1}] \
    [::questlog::scan::cutoff_for [dict create since 2026-04-01T13:37]]
check ceiling_dt $dt_hm \
    [::questlog::scan::ceiling_for [dict create until 2026-04-01T13:37]]

# ---- since_label: the one display-string home -----------------------
check label_all  all          [::questlog::scan::since_label all]
check label_week {1 week}     [::questlog::scan::since_label 7d]
check label_2w   {2 weeks}    [::questlog::scan::since_label 2w]
check label_6h   {6 hours}    [::questlog::scan::since_label 6h]
check label_90m  {90 minutes} [::questlog::scan::since_label 90m]
...
check label_abs \
    "since [string trim [clock format $abs_epoch -format %x -locale [::questlog::scan::time_locale]]]" \
